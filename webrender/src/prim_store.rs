...
impl GradientData {
    ...
    fn build(&mut self, src_stops: AuxIter<GradientStop>, reverse_stops: bool) {
        ...
        // This clone is free (just prevents progressing iterator)
        // The unwrap is fine because CSS requires at least two stops
        let mut cur_color = src_stops.clone().next().unwrap().color;
        ...
        if reverse_stops {
            ...
            if cur_idx != MIN_IDX {
                self.fill_colors(MIN_IDX, cur_idx, &cur_color, &cur_color);
            }
        } else {
            ...
            // Fill out any remaining entries in the gradient.
            if cur_idx != MAX_IDX {
                self.fill_colors(cur_idx, MAX_IDX, &cur_color, &cur_color);
            }
        }
    }
}
